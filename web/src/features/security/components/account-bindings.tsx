...
export function AccountBindings({ profile, onUpdate }: AccountBindingsProps) {
  ...
  const bindings: BindingItem[] = [
    ...
    },
    {
      ...
      // The unified provider requires the PKCE client configuration. Do not
      // expose a bind action that the backend will reject for bot-only legacy
      // deployments; those installations must migrate before binding.
      isEnabled: Boolean(
        status?.telegram_oauth && status?.telegram_oauth_configured
      ),
