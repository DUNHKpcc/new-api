...
/** Return all frontend query caches affected by an option update. */
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      if (data.success) {
        ...
        for (const queryKey of queryKeys.slice(1)) {
          queryClient.invalidateQueries({ queryKey: [queryKey] })
        }

        // Clear the persisted status snapshot whenever it was invalidated.
