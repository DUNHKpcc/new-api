...
func setupRelayRouterTestDB(t *testing.T) {
	...
	// Token authentication enriches every token with desktop-grant metadata;
	// keep the fixture schema aligned with that production query even when the
	// test does not exercise a desktop grant itself.
	require.NoError(t, model.DB.AutoMigrate(&model.User{}, &model.Token{}, &model.Ability{}, &model.DesktopGrant{}))
