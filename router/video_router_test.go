...
func TestGetOpenAIVideoRouteRendersJimengTask(t *testing.T) {
	...
	database := model.DB
	// ValidateUserToken now inspects desktop_grants while loading token
	// metadata.  Migrate that table in this isolated route fixture so a normal
	// non-desktop token is not turned into a database-error (HTTP 500).
	require.NoError(t, database.AutoMigrate(&model.User{}, &model.Token{}, &model.Channel{}, &model.Task{}, &model.DesktopGrant{}))
	t.Cleanup(func() {
