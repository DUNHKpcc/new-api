...
func ValidateConsoleSettings(settingsStr string, settingType string) error {
	...
	case "Announcements":
		return validateAnnouncements(settingsStr, false)
	case "GlobalNotifications":
		return validateAnnouncements(settingsStr, true)
	case "FAQ":
...
// NormalizeAnnouncements removes image payloads from ordinary announcements.
// Images are intentionally supported only by the separate global-notification
// stream; this helper gives migration and every write path one canonical way
// to clean legacy announcement values.
func NormalizeAnnouncements(settingsStr string) (string, error) {
	if strings.TrimSpace(settingsStr) == "" {
		return settingsStr, nil
	}

	list, err := parseJSONArray(settingsStr, "系统公告")
	if err != nil {
		return "", err
	}
	changed := false
	for _, announcement := range list {
		if _, exists := announcement["image"]; exists {
	...
	if err != nil {
		return "", fmt.Errorf("系统公告格式化失败：%w", err)
	}
...
func validateAnnouncements(announcementsStr string, allowImages bool) error {
	...
	for i, ann := range list {
		...
		if image, exists := ann["image"]; exists && allowImages {
...
func GetAnnouncements() []map[string]interface{} {
	value := GetConsoleSetting().Announcements
	if normalized, err := NormalizeAnnouncements(value); err == nil {
		value = normalized
	}
	return getPublicNotifications(value)
}
