...
// TestQuotaFromDecimalStrictRejectsOverflow keeps the fork's strict conversion
// regression while aligning its boundary with the upstream contract: the
// exact int32 maximum is representable, and only values above it are rejected.
func TestQuotaFromDecimalStrictRejectsOverflow(t *testing.T) {
	...
	quota, err = QuotaFromDecimalStrict(decimal.NewFromInt(MaxQuota + 1))
	assert.Zero(t, quota)
	var clamp *QuotaClamp
	require.ErrorAs(t, err, &clamp)
	assert.Equal(t, QuotaClampOverflow, clamp.Kind)
