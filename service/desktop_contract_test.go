...
func TestDesktopErrorContractMapping(t *testing.T) {
	tests := []struct {
		...
		{name: "wechat required", err: ErrDesktopWeChatRequired, status: http.StatusForbidden, code: DesktopErrorWeChatRequired},
		{name: "gift already claimed", err: ErrDesktopGiftAlreadyClaimed, status: http.StatusConflict, code: DesktopErrorGiftAlreadyClaimed},
...
func TestDesktopContractSchemaMatchesServerConstants(t *testing.T) {
	...
	serverCodes := []DesktopErrorCode{
		...
		DesktopErrorWeChatRequired,
		DesktopErrorGiftAlreadyClaimed,
