...
func TestFixedPriceBillingDatabaseMatrix(t *testing.T) {
	for _, dialect := range []struct {
		...
		t.Run(string(dialect.name), func(t *testing.T) {
			...
			// NewBillingSession checks whether the request token belongs to a
			// desktop grant before selecting its funding source. Keep that table
			// in this isolated matrix fixture so the check exercises the real
			// zero-grant path instead of surfacing a synthetic 500.
			require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.Channel{}, &model.DesktopGrant{}))
