...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "currency amount below limit", displayType: operation_setting.QuotaDisplayTypeUSD, amount: 4294, wantQuota: 2_147_000_000},
		{name: "currency amount above legacy int32 limit remains wallet-valid", displayType: operation_setting.QuotaDisplayTypeUSD, amount: 4295, wantQuota: 2_147_500_000},
		{name: "token amount preserves settlement truncation", displayType: operation_setting.QuotaDisplayTypeTokens, amount: 2_147_500_000, wantQuota: 2_147_500_000},
		{name: "token amount above legacy int32 range remains wallet-valid", displayType: operation_setting.QuotaDisplayTypeTokens, amount: 4_294_500_000, wantQuota: 4_294_500_000},
	}
...
func TestRequestAmountRejectsTopUpThatCannotBeSettled(t *testing.T) {
	...
	maxAmount := decimal.NewFromInt(common.MaxWalletQuota).
		Div(decimal.NewFromFloat(common.QuotaPerUnit)).
		Floor().IntPart()
	recorder := performEpayAmountRequest(t, 0, fmt.Sprintf(`{"amount":%d}`, maxAmount+1))
...
func TestRequestAmountRejectsTopUpThatWouldOverflowWallet(t *testing.T) {
	...
	useTopUpQuotaControllerSettings(t)
	require.NoError(t, model.DB.Create(&model.User{Id: 42, Username: "topup-capacity-user", Quota: common.MaxWalletQuota - 100_000, Status: common.UserStatusEnabled}).Error)
...
func TestRequestAmountAllowsNegativeBalanceToRepayDebt(t *testing.T) {
	useTopUpQuotaControllerTestDB(t)
	useTopUpQuotaControllerSettings(t)
	require.NoError(t, model.DB.Create(&model.User{Id: 43, Username: "topup-debt", Quota: -600_000, Status: common.UserStatusEnabled}).Error)
