...
func newAuditTestDatabase(t *testing.T, kind, dsn string) (*gorm.DB, string) {
	...
	if kind == "sqlite" {
		...
		// Match the production SQLite writer wait policy. The security tests
		// intentionally issue simultaneous writes; without a busy timeout the
		// second writer can fail with SQLITE_BUSY before the one-time proof
		// race is resolved. Keep the default transaction mode here because
		// several fixtures exercise nested read/write transactions.
