...
type User struct {
	...
	AffiliateSummary     *AffiliateUserAdminSummary `json:"affiliate_summary,omitempty" gorm:"-:all"`
}
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	idempotencyKey, err := common.GenerateRandomCharsKey(24)
	if err != nil {
		return err
	}
	_, err = TransferAffiliateSignupRewards(user.Id, int64(quota), "legacy_"+idempotencyKey)
	return err
}
...
// Post-creation sidebar and log tasks are handled after the transaction commits.
func (user *User) InsertWithTx(tx *gorm.DB, inviterId int) error {
	var signupRewardConfig AffiliateSignupRewardConfigSnapshot
	if inviterId > 0 {
		...
		signupRewardConfig = AffiliateSignupRewardConfigSnapshot{
			Enabled:            setting.RegistrationRewardEnabled && paymentComplianceConfirmedFromOptionValues(values),
	...
	return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
		...
		if err := createUserWithUniqueAffiliateCode(tx, user, func() string {
		...
		_, err := ApplyAffiliateReferralWithTx(tx, user, inviterId, signupRewardConfig)
		return err
	})
}
...
func (user *User) delete(identity *AuthSessionIdentity) error {
	...
	const maxSQLiteBusyRetries = 3
	for attempt := 0; ; attempt++ {
		nextAuthVersion = 0
		revokedDesktopGrants = nil
		err := DB.Transaction(func(tx *gorm.DB) error {
			if identity != nil {
		...
		if !common.UsingMainDatabase(common.DatabaseTypeSQLite) || !isSQLiteBusyError(err) || attempt >= maxSQLiteBusyRetries {
			return err
		}
		// A whole transaction is replayable here: no external state is
		// published until commit succeeds, and the caller's proof remains
		// consumed as required by the security contract.
