	...
	if shouldUpdateRedis(true, err) {
		gopool.Go(func() {
			if _, err := cacheInitToken(token); err != nil {
				common.SysLog("failed to initialize token cache: " + err.Error())
			}
		})
	}
	return &token, err
...
func DeleteTokenById(id int, userId int) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&DesktopGrant{}).
			...
			UpdateColumn("token_id", nil).Error; err != nil {
			return err
		}
		if err := tx.Model(&DesktopGrant{}).
			Where("user_id = ? AND status = ? AND codex_token_id = ?", userId, DesktopGrantStatusRevoked, id).
			UpdateColumn("codex_token_id", nil).Error; err != nil {
			return err
		}
...
func InvalidateTokenKeysCache(keys []string) error {
	...
	for _, key := range keys {
		...
		if err := invalidateTokenCacheForMutation(key); err != nil && firstErr == nil {
			firstErr = err
		}
	}
	return firstErr
